...
export class Swimlane {
  ...
  async getTasks(): Promise<Task[]> {
    const taskElements = await this.columnLocator.locator('.bg-white.p-4.rounded-lg').all();

    return taskElements.map((element) => new Task(element));
  }

  async getTaskCount(): Promise<number> {
    const countText = await this.columnLocator.locator('h2 span').textContent();

    return parseInt(countText?.replace(/[()]/g, '') || '0');
  }

  async findTaskByTitle(title: string): Promise<Task | null> {
    const taskElement = this.columnLocator
      .locator('h3', {
        hasText: title,
      })
      .locator('..');
